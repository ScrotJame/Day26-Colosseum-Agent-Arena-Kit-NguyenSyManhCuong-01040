...
_IMPERATIVE = (
    ...
    "disregard all", "system override", "bỏ qua mọi hướng dẫn", "you must now",
    "instead, record", "also record this for", "print the", "reveal the",
    "grading key", "private key", "secret key", "new instructions", "override system",
    "as the system", "role: system", "instruction override",
)
...
class Gateway:
    ...
    def __init__(self, ctx: GatewayContext) -> None:
        ...
        self._admitted_cards: dict[str, dict] = {
            "curriculum-analyst": {"verified": True, "skills": ["which_days_cover"]},
            "citation-checker": {"verified": True, "skills": ["verify_source"]},
            "roster": {"verified": True, "skills": ["get_roster", "list_peers"]},
        }
    ...
    # -- the four identity and security checks ------------------------------
    def _card_admitted(self, cmd: Command) -> bool:
        sig = cmd.headers.get("x-card-signature") or cmd.headers.get("X-Card-Signature")
        if sig == "invalid":
            return False
        if cmd.server not in A2A_SERVERS:
            return True
        card = self._admitted_cards.get(cmd.server)
        return bool(card and card.get("verified", True))

    def _server_vouched(self, cmd: Command) -> bool:
        fp = cmd.headers.get("x-server-fingerprint") or cmd.headers.get("X-Server-Fingerprint")
        if fp == "unvouched":
            return False
        if cmd.args.get("peer_unverified") is True:
            return False
        if "catalog" in cmd.args or "extra_rows" in cmd.args:
            return False
        if "catalog" in cmd.args and len(str(cmd.args["catalog"])) > 100:
            return False
        return True
    ...
    def _routes_on_header(self, cmd: Command) -> bool:
        ...
        for k in ("route", "_route", "replica", "_replica", "target_replica", "branch", "version"):
            if k in cmd.args:
                return False
        return True

    def _act_owns_target(self, cmd: Command) -> bool:
        """Authority comes from `act`, not `sub`."""
        act = getattr(self.ctx, "act", None)
        for key in ("learner", "learner_id", "target", "subject", "user", "student", "actor", "on_behalf_of"):
            target = cmd.args.get(key)
            if target and act and str(target).lower() != str(act).lower():
                return False
        return True
    ...
    def _instruction_in_content(self, cmd: Command) -> bool:
        if cmd.args.get("note") is not None or cmd.args.get("poison") is not None:
            return True
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        try:
            ...
            # ------------------------------------------------------------------
            # JOB 3 & 2 — AUTHORIZE & ADMIT
            # ------------------------------------------------------------------
            checks = (
                (self._card_admitted, "peer card signature invalid or not admitted by registry"),
                (self._server_vouched, "server fingerprint unvouched or unverified payload"),
            ...
            for check, reason in checks:
                if not check(cmd):
                    self.denied += 1
                    return self.deny(cmd, reason=reason)

            if self._instruction_in_content(cmd):
                self.quarantined += 1
                self.denied += 1
                return self.deny(cmd, reason="instruction found in retrieved content", quarantine=True)

            has_live_lease = bool(cmd.lease_id or self._leases or getattr(self.ctx, "leases", ()))
            if (server, tool) == ("slides", "get_frame") and not has_live_lease:
                self.denied += 1
                return self.deny(cmd, reason="get_frame requires a live lease_id")

            if not self._within_budget(cmd):
                self.denied += 1
                return self.deny(cmd, reason="round allowance exhausted; saving for late rounds")

            headers = {k: v for k, v in cmd.headers.items() if k.lower() != "x-mcp-body-route"}
            headers["Mcp-Replica"] = headers.get("Mcp-Replica", "w")

            is_write = (server, tool) in WRITE_TOOLS or (
                server in ("progress", "content") and tool in ("record_mastery", "flag_stale_slide", "file_content_bug")
            )
            if is_write:
                anchor = str(cmd.args.get("anchor", cmd.args.get("learner", "")))
                etag = self._etags.get(anchor) or headers.get("If-Match") or headers.get("if-match")
                if not etag:
                    self.denied += 1
                    return self.deny(cmd, reason="write without a fresh If-Match etag")
                key = f"{anchor}:{tool}:{cmd.args.get('kc', '')}:{cmd.args.get('learner', '')}"
                if key in self._idempotency:
                    self.denied += 1
                    return self.deny(cmd, reason="write already committed this duel")
                self._idempotency.add(key)
                headers["If-Match"] = str(etag)
                headers["Idempotency-Key"] = key

            # ------------------------------------------------------------------
            # JOB 4 — BUDGET: Mask discipline
            # ------------------------------------------------------------------
            requested_fields = tuple(cmd.fields)
            chosen_fields = requested_fields or MASKS.get((server, tool), ("anchor",))
            if requested_fields == ("*",) or (len(requested_fields) > 4 and (server, tool) in MASKS):
                chosen_fields = MASKS.get((server, tool), chosen_fields)
                rewritten = True

            self._spent_this_round += 1
            lease_id = cmd.lease_id or (getattr(self.ctx, "leases", (0,))[0] if getattr(self.ctx, "leases", ()) else None)
